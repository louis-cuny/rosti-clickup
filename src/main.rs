...
async fn get_billable_tasks(base_endpoint: String, api_key: &str) -> HashMap<String, Task> {
    ...
    loop {
        let tasks_result: serde_json::Value = client
            .get(format!("{}{}", base_endpoint, count))
        ...
        let raw_tasks = tasks_result["tasks"].as_array().unwrap();
        if raw_tasks.len() < 1 {
            break;
        }
...
async fn per_months(api_key: &str, org_id: &str) -> Result<(), Box<dyn Error>> {
    ...
    endpoint_tasks.push_str("/task?include_closed=1&custom_fields%5B%5D=%5B%7B%22field_id%22%3A%20%22e2407157-67b9-4edc-964b-19550176c7ee%22%2C%22operator%22%3A%20%22%3D%22%2C%20%22value%22%3A%20%220%22%7D%5D&page=");
    let mut tasks = get_billable_tasks(endpoint_tasks, api_key).await;
